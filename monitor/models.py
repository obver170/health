...
class Blood(models.Model):
    ...
    problem = models.BooleanField(verbose_name="Проблема?", default=False)
    # результат проверки
    fast_check = models.CharField(max_length=1000, default='')
    # дата и время проверки
    date = models.DateTimeField(auto_now=True)

    RBC = models.DecimalField(verbose_name="Эритроциты (× 10х12/л)", default="3.6", max_digits=4, decimal_places=2)
    MCV = models.DecimalField(verbose_name="Средний объем эритроцитов (фл или мкм3)",
                              default="81", max_digits=5, decimal_places=2)
    MCH = models.DecimalField(verbose_name="Средний уровень HGB в эритроците (пг)",
                              default="26", max_digits=5, decimal_places=2)
    MCHC = models.DecimalField(verbose_name="Средняя концентрация эритроцитов в гемоглобине (%)",
                               default="31", max_digits=5, decimal_places=2)
    RFV = models.DecimalField(verbose_name="Анизоцитоз эритроцитов (%)",
                              default="11.3", max_digits=5, decimal_places=2)

    HGB = models.DecimalField(verbose_name="Гемоглобин (г/л)", default="128", max_digits=5, decimal_places=2)
    HCT = models.DecimalField(verbose_name="Гематокрит (в % соотношении)",
                              default="128", max_digits=5, decimal_places=2)
    CP = models.DecimalField(verbose_name="Гемоглобин (г/л)", default="0.8", max_digits=4, decimal_places=2)
    PLT = models.DecimalField(verbose_name="Тромбоциты (× 10х9/л)", default="178", max_digits=5, decimal_places=2)
    ESR = models.DecimalField(verbose_name="СОЭ (мм/ч)", default="2", max_digits=4, decimal_places=2)
    MPV = models.DecimalField(verbose_name="Средний объем тромбоцитов (фл или мкм3)",
                              default="8", max_digits=4, decimal_places=2)
    WBC = models.DecimalField(verbose_name="Лейкоциты (× 10х9/л)", default="4", max_digits=4, decimal_places=2)
